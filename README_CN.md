...
`narrator-ai-web-backend` 是 NarratorAI 的可自托管 Flask 后端，用于模板化视频解说制作。它可以帮助用户自动完成电影、短剧等视频的解说制作。

用户在配套前端上传原始视频、影视片段、短剧素材或其他长视频素材后，选择一个解说模板。此前端会向本后端获取任务报价并提交任务；本后端负责模板定价、报价、钱包交易、用户级任务存储、云盘代理、任务编排和上游解说服务代理。

每个模板都可以配置自己的价格。用户在提交任务前可以看到本次任务价格，确认后再提交生成。连接的上游解说服务可以根据模板生成解说文案、剪辑脚本，并合成一条适合短视频平台发布的精简版解说视频。

这个项目适合用于电影解说、短剧解说、剧情混剪、内容工作室批量交付，以及想把解说模板做成可售卖商品的团队。

配套前端仓库：[NarratorAI-Studio/narrator-ai-web](https://github.com/NarratorAI-Studio/narrator-ai-web)。
